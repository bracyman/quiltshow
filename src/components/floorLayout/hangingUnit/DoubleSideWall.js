...
export class DoubleSideWall extends HangingUnit {
    ...
    getLines() {
        return [
            { top: 0, left: 0, length: this.getSize().length, angle: Facing.NORTH },
            { top: WallWidth, left: this.getSize().length, length: this.getSize().length, angle: Facing.SOUTH },
        ];
    }
    ...
    buildForm(announcer) {
        return (<div className="hanging-unit-form DoubleSideWall" aria-label="Single Side Wall">
            <div className="row" aria-label="Wall Name">
                {super.inputCell("Name", (<input type="text" name="name" id="name" value={this.name} onChange={this.update(announcer)} />))}
            </div>
